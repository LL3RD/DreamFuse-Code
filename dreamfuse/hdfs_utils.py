...
def listdir(path: str) -> List[str]:
    ...
    if path.startswith("hdfs://"):
        pipe = subprocess.Popen(
            args=["hdfs", "dfs", "-ls", path], shell=False, stdout=subprocess.PIPE
        )
...
def exists(file_path: str, re=False) -> bool:
    ...
    if file_path.startswith("hdfs"):
        if re:
            return (
                subprocess.run(
                    ["hdfs", "dfs", "-test", "-e", file_path + "*"], capture_output=True
                ).returncode
                == 0
            )
        return (
...
def remove(path: str):
    """
    remove file. Source and destination supports either hdfs or local path.
    """
    if path.startswith("hdfs://"):
        subprocess.run(["hdfs", "dfs", "-rm", "-r", "-f", "-skipTrash", path])
    else:
        shutil.rmtree(path)
